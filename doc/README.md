...
Важливою особливістю сервісів REST є те, що для пересилання стану представлення використовується протокол HTTP.
...
Endpoint (кінцева вершина) – це програма, підпрограма, чи функція, яка закінчує pipeline запиту, безпосередньо викликає CRUD операції для ресурсів та формує відповіді.
